...
// It prefers the channel head of the provider package's default channel over
// arbitrary bundles, avoiding picking an old or incompatible version.
func (r *Resolver) findGVKProvider(dep gvkDependency, excludeBundle string) string {
	// First, collect all bundles that provide the required GVK, grouped by package
		...
		for _, prop := range b.Properties {
			if prop.Type == "olm.gvk" {
				...
				if provided.Group == dep.Group &&
					...
					break // this bundle matches, no need to check more properties
				}
			}
		}
	}
	...
	// For each provider package, try to find the channel head of the default channel.
	// This ensures we pick the latest version rather than an arbitrary bundle.
	providerPackages := make(map[string]bool)
	for _, c := range candidates {
		providerPackages[c.packageName] = true
	}

	for pkgName := range providerPackages {
		pkg := r.fbc.GetPackage(pkgName)
		if pkg == nil {
			continue
		}
		ch := r.findChannel(pkgName, pkg.DefaultChannel)
		if ch == nil {
			continue
		}
		head, err := r.findChannelHead(ch)
		if err != nil {
			continue
		}
		// Verify the channel head actually provides the GVK
		for _, c := range candidates {
			if c.bundleName == head {
				return head
			}
		}
	}
	...
	return candidates[0].bundleName
}
