...
BINARIES   = kubectl-catalog oc-catalog
DIST_DIR   = dist
...
build-cross:
	...
	@for bin in $(BINARIES); do \
		CGO_ENABLED=0 GOOS=$(GOOS) GOARCH=$(GOARCH) go build \
			-ldflags "$(LDFLAGS) -s -w" \
			-o $(DIST_DIR)/$$bin-$(GOOS)-$(GOARCH)$(EXT) . ; \
	done
...
build-all: clean-dist
	...
	@for platform in $(PLATFORMS); do \
		...
		for bin in $(BINARIES); do \
			CGO_ENABLED=0 GOOS=$$GOOS GOARCH=$$GOARCH go build \
				-ldflags "$(LDFLAGS) -s -w" \
				-o $(DIST_DIR)/$$bin-$$GOOS-$$GOARCH$$EXT . || exit 1 ; \
		done ; \
	done
...
checksums: build-all
	@cd $(DIST_DIR) && shasum -a 256 kubectl-catalog-* oc-catalog-* > checksums.txt
